function [dict, offset_dict, maxLength_dict, string_dict] = lz77_encode_text(input, searchWindow, lookaheadBuffer)
...
for cursorPosition = searchBuffer_range(1):searchBuffer_range(2)
    dict = strcat(dict, '(', string(0), ',', string(0), ',', input(cursorPosition), ')');
    string_dict = strcat(string_dict, input(cursorPosition));
...
while cursorPosition <= length(input)
    ...
    dict = strcat(dict, '(', string(offset), ',', string(maxLength),',', input(cursorPosition-1), ')');
    % Add the character after matching sequences
    string_dict = strcat(string_dict, input(cursorPosition-1));
